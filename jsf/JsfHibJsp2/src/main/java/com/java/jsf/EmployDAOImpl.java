...
public class EmployDAOImpl {
	...
	public List<Employ> getEmployListInRange(int firstResult, int maxResults) {
        SessionFactory sf = SessionHelper.getConnection();
        Session session = sf.openSession();
        Criteria cr = session.createCriteria(Employ.class);
        cr.setFirstResult(firstResult);
        cr.setMaxResults(maxResults);
        cr.addOrder(Order.asc("empno")); // Add an order for pagination
        return cr.list();
    }
//	@SuppressWarnings("unchecked")
//	public List<Employ> showEmployDao(int firstRow, int rowCount) {
//		List<Employ> cdList = null;
//		try {
...
//			if (criteria != null) {
//				cdList = (List<Employ>) criteria.list();
//			}
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
