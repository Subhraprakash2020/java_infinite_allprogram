...
public class EmployeeDAOImpl implements EmployeeDAO {
	...
	public List<Employee> showEmployeeDao() throws ClassNotFoundException, SQLException {
		...
		while(rs.next()) {
			...
			employee.setDateOfJoin(rs.getDate("DateOfJion"));
	...
	@Override
	public List showLeaveMgridDao(int empId) throws ClassNotFoundException, SQLException {
		connection=ConnectionHelper.getConnection();
		String cmd ="Select * from Employee where ManagerId=?";
		pst=connection.prepareStatement(cmd);
		pst.setInt(1, empId);
		ResultSet rs = pst.executeQuery();
		Employee employ=null;
		List<Employee> leaveList = new ArrayList<Employee>();
		while(rs.next()) {
			employ = new Employee();
			employ.setEmpId(rs.getInt("EmpId"));
			leaveList.add(employ);
		}
		return leaveList;
	}
