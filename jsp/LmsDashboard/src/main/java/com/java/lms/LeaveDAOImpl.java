...
//	}
	...
	public String applyLeave(LeaveDetails leaveDetails) throws ClassNotFoundException, SQLException {
	    ...
	    if (workingDays <= 0) {
	        System.err.println("Start date cannot be greater than end date or leave duration falls on weekends.");
	        return " ";
	    }
	    
	    leaveDetails.setNoOfDays(workingDays);
	...
	private int calculateWorkingDays(Date startDate, Date endDate) {
	    ...
	    // Subtract 2 for each weekend day (Saturday and Sunday)
	    int weekends = (int) IntStream.range(0, workingDays).mapToObj(i -> startDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate().plusDays(i))
